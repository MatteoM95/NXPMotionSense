...
//Communication updates by Walter Dunckel 2016
  ...
  do {
    ...
    if (message != null) {
      ...
      if (list.length >= 4 && list[0].equals("O:")) {  
        Psi = float(list[1]); // convert to float yaw
...
{ 
  ...
  noStroke(); 
  rect(20, 470, 440, 50); 
  int Azimuth1=360+round(Azimuth); 
  textAlign(CORNER); 
  textSize(35); 
  fill(255); 
  ...
  textSize(40);
  fill(255);
  text(""+altitude+"", 250, 5, 2150, 60); //prints into right hand black box airspeed should be 250, 5, 3325, 60
  text(""+airspeed+"", 250, 5, 3325, 60); 
...
{ 
  ...
  rect(250, 0, 140, 40); //altitude box
  rect(-250, 0, 140, 40); //airspeed box
}
void CircularScale() 
{ 
...
{ 
  ...
  text("Bank:  "+round(Phi)+" Deg", 280, 411, 500, 60); 
}
void Borders() 
{ 
  noFill(); 
